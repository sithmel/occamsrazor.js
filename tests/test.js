...
test("Execute a function with 3 arguments (3)", function() {
  equals(this.hello("hello", " ", "world!") , "hello world!" ,"Sum is 6" );
});
...
module( "Pubsub and default stringvalidator", {
  setup: function (){
    ...
    this.pubsub.on('play', this.is_guitar, function (evt, instrument){
      return 'Strumming ' + instrument.instrument_name;
    });

    this.pubsub.on('play', this.is_electricguitar, function (evt, instrument){
      return instrument.instrument_name + ' solo';
    });

    this.pubsub.on('stop', this.is_guitar, function (evt, instrument){
      return 'stop playing';
    });

    this.pubsub.one('play', this.is_electricguitar, function (evt, instrument){
      return 'only once';
    });

    this.pubsub.on('riff', {instrument_name : 'guitar'}, function (evt, instrument){
      return 'strumming loud';
    });

    this.pubsub.on('riff', {instrument_name : 'electric guitar'}, function (evt, instrument){
      return 'smoking solo';
    });
...
test("Notify an event with 3 handler", function() {
  ...
  ok(outputs.indexOf('only once') !== -1, 'Called third handler');
  equals(outputs.length, 3, 'Just 3 outputs');

  ok(outputs2.indexOf('Strumming electric guitar') !== -1, 'Called first handler');
  ok(outputs2.indexOf('electric guitar solo') !== -1, 'Called second handler');
  equals(outputs2.length, 2, 'Just 2 outputs');
});

test("Notify an event with 1 handler", function() {

  var outputs = this.pubsub.trigger('play', this.guitar);
...
test("Notify an event with 1 handler (test default 'has' validator)", function() {

  var output1 = this.pubsub('riff', this.guitar);
  var output2 = this.pubsub('riff', this.electricguitar);
  equals(output1, 'strumming loud');
  equals(output2, 'smoking solo');
...
test("adding a constructor function", function() {
...
test("more than one adapter", function() {
  var isAnything = occamsrazor.validator();
  var hasWidth = isAnything.match({'width': undefined});
  var hasHeight_hasWidth = hasWidth.match({'height': undefined});
...
module( "validator match" );

test("single/multiple -match-", function() {
  var isAnything = occamsrazor.validator();
  var hasWidth = isAnything.match({'width': undefined});
  var hasHeight_hasWidth = isAnything.match({'width': undefined, 'height': undefined});
...
test("single/multiple -match- object", function() {
  var isAnything = occamsrazor.validator();
  var hasWidth10 = isAnything.match({'width': '10'});
  var hasX10 = isAnything.match({
    center: {
      x: "10", y: undefined
    }
  });
...
test("-match- can use functions", function() {
  var isAnything = occamsrazor.validator();
  var hasWidthbetween5and10 = isAnything.match({width: function (w){
    return w >= 5 && w <=10;
  }});
...
test("-match- can use arrays", function() {
  var isAnything = occamsrazor.validator();
  var hasWidthAndHeight = isAnything.match(['width', 'height']);

  equal(hasWidthAndHeight({width: 8, height: 10}), 2, "match");
