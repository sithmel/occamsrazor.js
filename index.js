...
var decorateAndFilter = function (args, functions) {
  var i
  var result
  var results = []
...
// functions is a list of obj (func, validators)
var filterAndSort = function (args, functions, onlyOne) {
  var results = decorateAndFilter(args, functions)
...
var _occamsrazor = function (adapterFuncs, stickyArgs) {
  ...
  var stickyArguments = stickyArgs || []

  var occamsrazor = function () {
    var args = Array.prototype.slice.call(arguments)
    var funcs = filterAndSort(args, functions, true)
  ...
  var getAdd = function (times) {
    return function () {
      ...
      for (var i = 0; i < stickyArguments.length; i++) {
        funcs = filterAndSort(stickyArguments[i].args, _funcs)
  ...
  occamsrazor.size = function size () {
    var args = Array.prototype.slice.call(arguments)
    var funcs = args.length ? filterAndSort(args, functions) : functions
    return funcs.length
  }
  ...
  occamsrazor.all = occamsrazor.triggerSync = function all () {
    var args = Array.prototype.slice.call(arguments)
    var funcs = filterAndSort(args, functions)
  ...
  var adapter = function (_registry) {
    return function (functionName) {
      if (!(functionName in _registry)) {
        _registry[functionName] = _occamsrazor()
      }
